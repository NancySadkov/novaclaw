...
Both `action` and `resource` use novaclaw's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.novaclaw` files is intentionally deferred until `.novaclaw` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether novaclaw permits an operation through its managed execution paths.
