...
The novaclaw server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@novaclaw-ai/llm` tool loop after replacing its
...
- plugins get global "novaclaw" instance like in that post i showed
- novaclaw instance has stuff like `novaclaw.session.prompt()` or
  `novaclaw.tool.register({...})`
...
- decide whether to preserve deprecated `@novaclaw-ai/llm` orchestration exports
