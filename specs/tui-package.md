...
Move the canonical NovaClaw terminal application from
`packages/novaclaw/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/novaclaw`,
`packages/cli`, or `@novaclaw/core`.

The SDK is the TUI's NovaClaw boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/novaclaw`.
- Do not replace private `packages/novaclaw` imports with `@novaclaw/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@novaclaw/tui`.
...
- NovaClaw domain data displayed by the TUI
...
```text
packages/novaclaw/src/cli/cmd/tui
```

Its private dependency on `packages/novaclaw` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/novaclaw/src/*`.
...
Status: Completed. The private `@novaclaw/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@novaclaw/tui`.
...
- The package has no dependency on `novaclaw`, `@novaclaw/cli`, or
  `@novaclaw/core`.
...
primitives now live in `@novaclaw/tui`. The legacy host consumes explicit
...
and build inputs through `@novaclaw/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads NovaClaw globals or process state;
...
Replace process-global NovaClaw state with resolved TUI inputs.
...
`@novaclaw/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/novaclaw` initially.
...
canonical in `@novaclaw/tui`. Configured references resolve through the new
...
adapters rather than NovaClaw domain implementations.

Make the SDK the only NovaClaw domain boundary used by the TUI.
...
- Domain-facing TUI code imports NovaClaw data and operations only from
  `@novaclaw/sdk`.
...
`@novaclaw/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/novaclaw`.
...
- The package has no imports from `packages/novaclaw`, `packages/cli`, or
  `@novaclaw/core`.
...
public `@novaclaw/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/novaclaw`.
- Keep the legacy embedded worker and server startup in `packages/novaclaw`.
- Change those adapters to load config, create transport inputs, and call the
  public `@novaclaw/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/novaclaw`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@novaclaw/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@novaclaw/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for NovaClaw domain state.
...
cd packages/novaclaw && bun typecheck
...
rg '@novaclaw/core|packages/novaclaw|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/novaclaw/src packages/novaclaw/test
```
...
- Build the current-platform `packages/novaclaw` binary.
