...
// etc.).
//
// The visual language mirrors the NovaClaw app: the design tokens are a curated
// subset of the brand semantic tokens in `packages/ui/src/styles/theme.css`, and
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `NovaClaw is now connected to ${escapeHtml(provider)}.` : "NovaClaw is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `NovaClaw couldn't finish connecting to ${escapeHtml(provider)}.`
        : "NovaClaw couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from NovaClaw.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · NovaClaw</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("NovaClaw couldn't finish connecting to "+PROVIDER+"."):"NovaClaw couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from NovaClaw."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("NovaClaw is now connected to "+PROVIDER+"."):"NovaClaw is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// NovaClaw wordmark — same path geometry as packages/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="NovaClaw" role="img">
