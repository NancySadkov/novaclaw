...
// dispatches) ∪ the MCP ExternalCommandSource. Collision precedence mirrors the
// dispatch fallback order: CommandV2 > MCP. Each entry carries the presentation metadata
// the composer renders (`source` badge + `$1..$N` argument hints); MCP templates resolve
// lazily at dispatch, so their listed template is "". Shared by the V2 `/api/command`
// route and the legacy instance `/command` route so the two can never diverge again.
...
export const list: Effect.Effect<
  Command.Info[],
  never,
  CommandV2.Service | ExternalCommandSource.Service
> = Effect.gen(function* () {
