...
// five drives on every instance in the world — a change that makes the product quietly worse and
...
describe("resolve", () => {
  ...
  test("a malformed stored block leaves every drive ON", () => {
    const enabled = {
      reground: true,
      set: true,
      children: true,
  ...
  test("all five can be off at once — the fully unaided configuration", () => {
    expect(
