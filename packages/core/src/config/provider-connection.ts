...
/** Thirteen minutes and twenty seconds without a single streamed event means the connection is no longer useful. */
export const DEFAULT_STALL_TIMEOUT_MS = 800_000
...
export class Info extends Schema.Class<Info>("ConfigV2.ProviderConnection")({
  ...
  stall_timeout_ms: ConfigAnnotation.withDefault(
    StallTimeout.pipe(Schema.optional).annotate({
      description:
        "Maximum time to wait without any streamed provider event before NovaClaw stops the attempt (default 800000 ms; 30000-1800000)",
    }),
