...
export function introspectPrompt(input: {
  ...
  const system = [
    ...
    "  - consumes / produces: artifact-id slugs this step READS / WRITES (lowercase, e.g. `add.c`). Declare them: every consumed id must be produced by an EARLIER substep or already exist — the harness REJECTS a dangling reference.",
    "  - success: the human-readable check (what 'done' means).",
