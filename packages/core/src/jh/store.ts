...
interface SerializedState {
  readonly controller: ReturnType<typeof JhController.encode>
...
const serialize = (state: JhEngine.State): SerializedState => ({
  controller: JhController.encode(state.controller),
  ...
  return db
    .transaction((tx) =>
      Effect.gen(function* () {
        ...
        yield* tx.delete(JhArtifactTable).where(eq(JhArtifactTable.planID, input.id)).run().pipe(Effect.orDie)
        if (input.state.artifacts.length > 0) {
          ...
          yield* tx.insert(JhArtifactTable).values(rows).run().pipe(Effect.orDie)
...
export function load(
  ...
  return db
    .transaction((tx) =>
      Effect.gen(function* () {
        const plan = yield* tx.select().from(JhPlanTable).where(eq(JhPlanTable.id, id)).get().pipe(Effect.orDie)
        ...
        const state: JhEngine.State = {
          controller: JhController.decode(s.controller),
