...
const instanceTmp = (data: string) => path.join(data, "tmp")
...
export type DirectoryStatus =
  ...
  | { readonly state: "ok" }
  /** The chosen home could not be created. Named, not fatal. */
...
const dirs = (): Xdg.Dirs => {
  ...
  if (found === undefined || Object.values(found).some((v) => typeof v === "string" && Xdg.isSuspect(v))) {
    throw new Error("NovaClaw could not determine a usable instance home. Pass --home <dir> to select one.")
  }
  const chosen = found
  ...
  // unrecoverable boot defect. It now reports failed directories and retains the selected home.
  const failures = ensureDirectories(directoriesOf(chosen, instanceTmp(chosen.data)))
  if (failures.length > 0) {
    status = { state: "degraded", failures }
  }

  if (status.state === "degraded") {
    // The instance still comes up — a running process that can be
    ...
    console.error(
      `[novaclaw] WARNING: NovaClaw could not create these directories under its selected home: ` +
