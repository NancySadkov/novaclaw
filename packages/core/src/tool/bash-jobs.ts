...
// stop its jobs. In-memory + location-scoped by design (v1): jobs die with the
// location/server — durable status + restart recovery stay a documented follow-up
// (see the TODO ledger in tool/bash.ts).
  ...
  Effect.gen(function* () {
    ...
    const find = (id: string, owner: string) =>
      Effect.suspend(() => {
        ...
        if (!job || job.owner !== owner) return Effect.fail(new JobNotFoundError({ id }))
    ...
    const status: Interface["status"] = Effect.fn("BashJobs.status")(function* (id, owner) {
      purge()
      return snapshot(yield* find(id, owner))
    })
