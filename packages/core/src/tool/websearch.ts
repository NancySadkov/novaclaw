...
export class ConfigService extends Context.Service<ConfigService, Config>()("@novaclaw/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.NOVACLAW_WEBSEARCH_PROVIDER === "exa" || process.env.NOVACLAW_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.NOVACLAW_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("NOVACLAW_EXPERIMENTAL") || truthy("NOVACLAW_ENABLE_EXA") || truthy("NOVACLAW_EXPERIMENTAL_EXA"),
    enableParallel: truthy("NOVACLAW_ENABLE_PARALLEL") || truthy("NOVACLAW_EXPERIMENTAL_PARALLEL"),
