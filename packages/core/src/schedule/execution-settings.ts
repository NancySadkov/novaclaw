 ...
 *
 * 🔴 NC-REL-027 — `agent` is a free-text string that no write boundary resolved. A typo persisted
 * happily and failed hours later inside a detached session, on work nobody was present to retry:
 * *"scheduled work is explicitly unattended, so delayed validation converts a simple, repairable form
 * error into a missed report nobody is there to notice."*
 *
 * ⚠️ Pure, and it takes the KNOWN set rather than resolving it. The interesting decisions are all about
 * wording and about which absences are legal; resolving a roster through a location would put that
 * behind a service graph and test none of it.
 */
 ...
 * arrive by different roads: a location-scoped roster with no ambient location, or an instance
 ...
 *
 * ⚠️ ABSENT is always legal. Omitting an agent means "the instance decides at fire time", which is a
 * standing choice a user is entitled to make; refusing it would force every schedule to name a
 * colleague that might be retired before it fires. Only a NAMED thing that does not exist is refused
 * — the difference between "I did not choose" and "I chose something that is not there".
 *
 * ⚠️ `null` is absent too. The update payload uses it to CLEAR the field, and treating a clear as a
 * bad value would make the only way to unset an agent impossible.
 *
 * ⚠️ An UNRESOLVED set refuses nothing. A refusal that cannot say what to write instead is a dead end
