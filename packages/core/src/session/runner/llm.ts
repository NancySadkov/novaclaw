...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
     ...
     * The prompt as the ONE epoch source.
     *
     * ⚠️ An empty render throws (`SystemContext.requireText`), so the "no system prompt at all" case —
     * a pure Chat with empty job instructions — returns `SystemContext.empty`, which renders to the
     * empty baseline the runner then drops.
     *
     * ⚠️ The comparator compares the RENDERED TEXT, not "always equal". The epoch still reuses the
     * bytes on a casual turn because the text is identical, but `prepareTurn` also reads the stored
    ...
    const prepareTurn = Effect.fn("SessionRunner.prepareTurn")(function* (
      ...
      // ONE prompt source. Render it now, compare with what the epoch stored, and tell the epoch to
