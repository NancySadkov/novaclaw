...
describe("the session-runner claims ledger", () => {
  ...
  test("the remaining claims are the size of the job left", () => {
    ...
    expect(remaining.length).toBe(33)
  })

  test("the remaining declarations are 33 tests, and the ledger knows why", () => {
    ...
    //
    // It was 77 → 83 when the ledger landed; forty-four ported claims make it 33 → 33 — the three PARAMETERISED ones went in this slice, so declarations and tests are now equal for the first time.
    const remaining = CLAIMS.filter((claim) => STATUS[claim] === undefined)
    const parameterised = remaining.filter((claim) => claim.includes("${kind}"))
    expect(parameterised.length).toBe(0)
    const FRAGMENT_KINDS = 3
    expect(remaining.length - parameterised.length + parameterised.length * FRAGMENT_KINDS).toBe(33)
  })
})
