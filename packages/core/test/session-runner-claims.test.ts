...
describe("the session-runner claims ledger", () => {
  ...
  test("the remaining declarations are 66 tests, and the ledger knows why", () => {
    ...
    //
    // It was 77 → 83 when the ledger landed; seventeen ported claims (none parameterised) make it 60 → 66.
    const remaining = CLAIMS.filter((claim) => STATUS[claim] === undefined)
    const parameterised = remaining.filter((claim) => claim.includes("${kind}"))
    expect(parameterised.length).toBe(3)
    const FRAGMENT_KINDS = 3
    expect(remaining.length - parameterised.length + parameterised.length * FRAGMENT_KINDS).toBe(66)
  })
})
