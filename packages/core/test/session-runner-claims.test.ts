...
describe("the session-runner claims ledger", () => {
  ...
  test("the remaining claims are the size of the job left", () => {
    ...
    expect(remaining.length).toBe(59)
  })

  test("the remaining declarations are 65 tests, and the ledger knows why", () => {
    ...
    //
    // It was 77 → 83 when the ledger landed; eighteen ported claims (none parameterised) make it 59 → 65.
    const remaining = CLAIMS.filter((claim) => STATUS[claim] === undefined)
    const parameterised = remaining.filter((claim) => claim.includes("${kind}"))
    expect(parameterised.length).toBe(3)
    const FRAGMENT_KINDS = 3
    expect(remaining.length - parameterised.length + parameterised.length * FRAGMENT_KINDS).toBe(65)
  })
})
