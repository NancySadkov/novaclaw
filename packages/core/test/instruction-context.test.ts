  ...
  it.effect("honors the project instruction opt-out", () =>
    Effect.gen(function* () {
      ...
      yield* InstructionContext.Service.pipe(
        ...
        Effect.ensuring(
          Effect.sync(() => {
            if (previous === undefined) delete process.env.NOVACLAW_DISABLE_PROJECT_CONFIG
            else process.env.NOVACLAW_DISABLE_PROJECT_CONFIG = previous
          }),
        ),
      )
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────
// 🗑️ `describe("InstructionContext — project exclusions")` stood here. Its subject was the folder's `novaclaw.json`, retired
