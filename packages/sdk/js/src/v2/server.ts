...
export async function createNovaclawServer(options?: ServerOptions) {
  ...
  const proc = launch(`novaclaw`, args, {
    env: {
      ...process.env,
      NOVACLAW_CONFIG_CONTENT: JSON.stringify(options.config ?? {}),
    },
  })
  let clear = () => {}
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    proc.stdout?.on("data", (chunk) => {
      ...
      for (const line of lines) {
        if (line.startsWith("novaclaw server listening")) {
...
export function createNovaclawTui(options?: TuiOptions) {
  const args = []
