      ...
      Effect.gen(function* () {
        return Config.of(
          yield* EffectConfig.all({
            ...
            username: EffectConfig.string("NOVACLAW_SERVER_USERNAME").pipe(EffectConfig.withDefault("opencode")),
          }),
        )
      }),
    )
...
export function header(credentials?: Credentials) {
  ...
  return `Basic ${Buffer.from(`${credentials?.username ?? process.env.NOVACLAW_SERVER_USERNAME ?? "opencode"}:${password}`).toString("base64")}`
}
