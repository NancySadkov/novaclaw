...
export const page = Effect.fn("MessageV2.page")(function* (input: {
  ...
  // S7: this page serves LEGACY Message/Part rows only (the V1 engine's sessions). A V2-native
  // session's transcript lives in the disjoint session_message table and is served by the native
  // `GET /api/session/{id}/message` — the F0-era projection of native rows into the V1 WithParts
  // shape (message-v2-native.ts) retired with the V1 render vocab.
  if (rows.length === 0) {
    const row = yield* db
  ...
  return {
    items,
    more,
    cursor: more && tail ? cursor.encode({ id: tail.id as MessageID, time: tail.time_created }) : undefined,
  }
})
