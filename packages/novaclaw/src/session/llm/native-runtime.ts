...
import type { LLMClientShape } from "@novaclaw/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("novaclaw"))
    return { type: "unsupported", reason: "provider is not openai, novaclaw, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @novaclaw/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains novaclaw-owned. The native runtime only adapts
      // the @novaclaw/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
