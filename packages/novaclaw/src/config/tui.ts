...
import { isRecord } from "@novaclaw/tui/util/record"
import { Global } from "@novaclaw/core/global"
import { FSUtil } from "@novaclaw/core/fs-util"
import { CurrentWorkingDirectory } from "./tui-cwd"
import { ConfigPlugin } from "@/config/plugin"
import { TuiKeybind } from "@novaclaw/tui/config/keybind"
...
import { TuiConfig } from "@novaclaw/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@novaclaw/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old novaclaw.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.novaclaw`
  // folders between cwd and home, and NOVACLAW_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.NOVACLAW_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit NOVACLAW_TUI_CONFIG override, if set.
  if (Flag.NOVACLAW_TUI_CONFIG) {
    const configFile = Flag.NOVACLAW_TUI_CONFIG
  ...
  // 4. `.novaclaw` directories (and NOVACLAW_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".novaclaw") || dir === Flag.NOVACLAW_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".novaclaw") && dir !== Flag.NOVACLAW_CONFIG_DIR) continue
