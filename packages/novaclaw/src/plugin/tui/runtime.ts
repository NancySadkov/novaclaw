...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@novaclaw/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@novaclaw/tui/util/error"
...
import { hasTheme, upsertTheme } from "@novaclaw/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@novaclaw/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@novaclaw/core/v1/config/plugin"
import { createCommandShim } from "@novaclaw/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@novaclaw/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".novaclaw"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".novaclaw", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".novaclaw", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.NOVACLAW_PURE ? [] : pluginOrigins
    if (Flag.NOVACLAW_PURE && pluginOrigins.length) {
    }
