...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall novaclaw and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall NovaClaw")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g novaclaw",
      pnpm: "pnpm uninstall -g novaclaw",
      bun: "bun remove -g novaclaw",
      yarn: "yarn global remove novaclaw",
      brew: "brew uninstall novaclaw",
      choco: "choco uninstall novaclaw",
      scoop: "scoop uninstall novaclaw",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "novaclaw", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using NovaClaw!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# novaclaw") || content.includes(".novaclaw/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".novaclaw/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".novaclaw/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".novaclaw"))
    ) {
      continue
    }
