...
Trim `packages/novaclaw/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/novaclaw/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/novaclaw`.
- `[x]` `bun typecheck` from `packages/novaclaw`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/novaclaw/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/novaclaw`.
- `[x]` `bun dev generate > /tmp/novaclaw-openapi.json` from `packages/novaclaw`.
...
- Edit `packages/novaclaw/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/novaclaw-openapi.json` from `packages/novaclaw`.
...
- Check source schemas in files like `packages/novaclaw/src/session/schema.ts`, `packages/novaclaw/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/novaclaw/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/novaclaw/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
