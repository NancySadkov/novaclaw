...
Delete `packages/novaclaw/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/novaclaw/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/novaclaw/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/novaclaw`
- TUI plugin loader tests in `packages/novaclaw` if runtime plugin API wiring changed
