...
- Author package entrypoint is `@novaclaw/plugin/tui`.
...
```json
{
  "$schema": "https://novaclaw.app/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/novaclaw-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@novaclaw/plugin/tui`.
- `@novaclaw/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@novaclaw/plugin/tui"
...
- The value is a semver range checked against the running NovaClaw version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.novaclaw` is absent, no check is performed (backward compatible).
...
- `novaclaw plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `novaclaw plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.novaclaw` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.novaclaw`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.novaclaw`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.novaclaw` dir or `NOVACLAW_CONFIG_DIR`, NovaClaw installs `@novaclaw/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@novaclaw/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus NovaClaw's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
NovaClaw registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@novaclaw/plugin/tui`.
...
- `message` is required; `title` defaults to `"novaclaw"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `novaclaw.default`.
...
- Local plugins persist installed themes under the local `.novaclaw/themes` area near the plugin config source.
...
- `--pure` / `NOVACLAW_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.novaclaw/plugins/tui-smoke.tsx`
- Local vim plugin: `.novaclaw/plugins/tui-vim.tsx`
- Local smoke config: `.novaclaw/tui.json`
- Local smoke theme: `.novaclaw/plugins/smoke-theme.json`
