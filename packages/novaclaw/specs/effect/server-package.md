...
Practical reference for a future `packages/server` split after the novaclaw
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/novaclaw`.
...
Until enough shared service code lives outside `packages/novaclaw`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/novaclaw`

It should not import `packages/novaclaw` services while `packages/novaclaw`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on novaclaw-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/novaclaw` implementation details.
