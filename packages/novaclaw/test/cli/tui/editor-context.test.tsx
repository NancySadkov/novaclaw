...
import { EditorContextProvider, useEditorContext, type EditorIntegration } from "@novaclaw/tui/context/editor"
...
function mountEditorContext(WebSocketImpl?: typeof WebSocket) {
  ...
  createRoot((nextDispose) => {
    ...
    const value = process.env.CLAUDE_CODE_SSE_PORT || process.env.NOVACLAW_EDITOR_SSE_PORT
    return (
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.NOVACLAW_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with NOVACLAW_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.NOVACLAW_EDITOR_SSE_PORT = "4020"
