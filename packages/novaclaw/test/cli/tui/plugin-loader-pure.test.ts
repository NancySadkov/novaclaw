...
`,
  ...
  const pure = process.env.NOVACLAW_PURE
  const meta = process.env.NOVACLAW_PLUGIN_META_FILE
  process.env.NOVACLAW_PURE = "1"
  process.env.NOVACLAW_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.NOVACLAW_PURE
    } else {
      process.env.NOVACLAW_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.NOVACLAW_PLUGIN_META_FILE
    } else {
      process.env.NOVACLAW_PLUGIN_META_FILE = meta
    }
  }
})
