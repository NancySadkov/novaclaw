...
import type { QuestionRequest } from "@novaclaw/sdk/v2"
import { NovaclawKeymapProvider, registerNovaclawKeymap } from "@novaclaw/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerNovaclawKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerNovaclawKeymap(keymap, renderer, tuiConfig)
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerNovaclawKeymap(keymap, renderer, tuiConfig)
