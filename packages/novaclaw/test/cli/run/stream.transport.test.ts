...
import { NovaclawClient, type GlobalEvent } from "@novaclaw/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<NovaclawClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<NovaclawClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<NovaclawClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<NovaclawClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<NovaclawClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://novaclaw.test"),
...
function sdk(
  input: {
    ...
    subscribe?: NovaclawClient["event"]["subscribe"]
    globalEvent?: NovaclawClient["global"]["event"]
    promptAsync?: NovaclawClient["session"]["promptAsync"]
    status?: NovaclawClient["session"]["status"]
    messages?: NovaclawClient["session"]["messages"]
    children?: NovaclawClient["session"]["children"]
    permissions?: NovaclawClient["permission"]["list"]
    questions?: NovaclawClient["question"]["list"]
  } = {},
) {
  const client = new NovaclawClient()

  const subscribe: NovaclawClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: NovaclawClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: NovaclawClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: NovaclawClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: NovaclawClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: NovaclawClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: NovaclawClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: NovaclawClient["question"]["list"] = input.questions ?? (() => ok([]))
