...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const novaclawFile = path.join(tmp, ".git", "novaclaw")
      expect(yield* Effect.promise(() => Bun.file(novaclawFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "novaclaw")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "novaclaw")
      const wrongCache = path.join(parentDir, ".git", "novaclaw")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "novaclaw")
      const cacheB = path.join(bareB, "novaclaw")
      const wrongCache = path.join(parentDir, ".git", "novaclaw")
