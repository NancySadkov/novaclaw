...
- `it.effect(...)` — pure Effect behavior with `TestClock` and
  `TestConsole`.
- `it.instance(...)` — service behavior that needs one scoped novaclaw
  instance.
...
9. Run the focused test file and `bun typecheck` from `packages/novaclaw`.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep\|withTestInstance" -- packages/novaclaw/test
```
