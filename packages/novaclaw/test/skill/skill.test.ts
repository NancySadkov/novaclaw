...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.NOVACLAW_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.NOVACLAW_TEST_HOME = prev
      }),
  )
...
describe("skill", () => {
  ...
  it.live("discovers skills from .novaclaw/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".novaclaw", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
...
`,
          ...
          const list = (yield* skill.all()).filter((s) => s.location !== SkillBuiltin.LOCATION)
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".novaclaw", "skill", "dir-skill", "SKILL.md"),
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".novaclaw", "skill", "dir-skill"))
  ...
  it.live("discovers multiple skills from .novaclaw/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".novaclaw", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".novaclaw", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".novaclaw", "skill", "no-frontmatter", "SKILL.md"),
              `# No Frontmatter
...
`,
          ...
          const skill = yield* Skill.Service
          expect((yield* skill.all()).filter((s) => s.location !== SkillBuiltin.LOCATION)).toEqual([])
        }),
      { git: true },
    ),
  )
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".novaclaw", "skill", "manual-skill", "SKILL.md"),
              `---
name: manual-skill
---
