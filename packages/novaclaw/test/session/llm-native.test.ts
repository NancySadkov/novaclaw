...
import { LLMEvent, ToolFailure } from "@novaclaw/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@novaclaw/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("novaclaw"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("novaclaw") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("novaclaw") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, novaclaw, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored novaclaw auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("novaclaw") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("novaclaw"),
