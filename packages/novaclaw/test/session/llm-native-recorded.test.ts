...
import { HttpRecorderInternal } from "@novaclaw/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@novaclaw/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@novaclaw/llm/route"
import { Env } from "@/env"
...
const RECORDED_SCENARIOS = [
  {
    ...
    config: (model) =>
      providerConfig({
        ...
        model,
        options: {
          apiKey: envValue("NOVACLAW_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    config: (model) =>
      providerConfig({
        ...
        model,
        options: {
          apiKey: envValue("NOVACLAW_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("NOVACLAW_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
