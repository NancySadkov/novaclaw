...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".novaclaw")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("hides task background parameter unless experimental background subagents are enabled", () =>
    Effect.gen(function* () {
      ...
      const task = (yield* registry.tools({
        providerID: ProviderV2.ID.novaclaw,
  ...
  it.instance("loads tools from .novaclaw/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const novaclaw = path.join(test.directory, ".novaclaw")
      const tool = path.join(novaclaw, "tool")
  ...
  it.instance("ignores non-tool exports in .novaclaw/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
  ...
  // came in through `plugin.list()` — `oh-my-novaclaw` was registering a tool
  ...
  it.instance("loads tools from .novaclaw/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const novaclaw = path.join(test.directory, ".novaclaw")
      const tools = path.join(novaclaw, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".novaclaw", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const novaclaw = path.join(test.directory, ".novaclaw")
        const customTools = path.join(novaclaw, "tools")
        const plugin = path.join(novaclaw, "node_modules", "@novaclaw-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(novaclaw, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@novaclaw/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".novaclaw", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(novaclaw, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@novaclaw/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(novaclaw, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(novaclaw, "node_modules", "cowsay")
