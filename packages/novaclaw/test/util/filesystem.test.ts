...
import { symlinkFixtureTest } from "../fixture/symlink"
...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "novaclaw.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "novaclaw.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "novaclaw.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "novaclaw.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["novaclaw.json", "novaclaw.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "novaclaw.jsonc"),
      ])
    })
  })
  ...
  describe("resolve()", () => {
    ...
    symlinkFixtureTest("resolves symlinked directory to canonical path", async () => {
      await using tmp = await tmpdir()
    ...
    symlinkFixtureTest("throws ELOOP on symlink cycle", async () => {
      await using tmp = await tmpdir()
