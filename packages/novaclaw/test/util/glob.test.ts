...
describe("Glob", () => {
  describe("scan()", () => {
    ...
    symlinkFixtureTest("does not follow symlinks by default", async () => {
      await using tmp = await tmpdir()
    ...
    symlinkFixtureTest("follows symlinks when symlink option is true", async () => {
      await using tmp = await tmpdir()
      await fs.mkdir(path.join(tmp.path, "realdir"))
      await fs.writeFile(path.join(tmp.path, "realdir", "file.txt"), "", "utf-8")
      await fs.symlink(
  ...
  describe("match()", () => {
    ...
    test("matches dot files", () => {
      ...
      expect(Glob.match("**/*.md", ".forge/README.md")).toBe(true)
    })
