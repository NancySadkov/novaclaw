...
describe("plugin.install.task", () => {
  test("writes the server config entry", async () => {
  ...
  test("preserves JSONC comments when adding plugins to the server config", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".novaclaw")
    const server = path.join(cfg, "novaclaw.jsonc")
...
`,
  ...
  test("preserves JSONC comments when force replacing plugin version", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".novaclaw", "novaclaw.jsonc")
...
`,
  ...
  test("does not change configured package version without force", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".novaclaw", "novaclaw.json")
    await fs.mkdir(path.dirname(cfg), { recursive: true })
  ...
  test("writes to global scope when global flag is set", async () => {
    ...
    expect(await Filesystem.exists(path.join(global, "novaclaw.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".novaclaw", "novaclaw.jsonc"))).toBe(false)
  })
  ...
  test("writes local scope under directory when vcs is not git", async () => {
    ...
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".novaclaw", "novaclaw.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(worktree, ".novaclaw", "novaclaw.jsonc"))).toBe(false)
  })
  ...
  // The TUI is gone, so a `tui` plugin target is gone with it. These two pin the absence: a manifest
  // that only offers the retired surfaces is REJECTED, and no `tui.jsonc` is ever written. Without
  // them, re-adding either arm compiles green and silently writes a config file nothing reads.
  test("rejects a manifest that only declares the retired tui export", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, undefined, undefined, { exports: { "./tui": "./tui.js" } })
    const run = createPlugTask(
      {
        mod: "acme@1.2.3",
      },
    ...
    expect(await Filesystem.exists(path.join(tmp.path, ".novaclaw", "tui.jsonc"))).toBe(false)
    expect(await Filesystem.exists(path.join(tmp.path, ".novaclaw", "novaclaw.jsonc"))).toBe(false)
  })

  test("rejects a manifest that only declares oc-themes", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, undefined, undefined, { "oc-themes": ["themes/forest.json"] })
  ...
  test("returns false when manifest declares no supported target", async () => {
    await using tmp = await tmpdir()
