...
import type { HostPluginApi } from "@novaclaw/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createNovaclawClient({
  ...
  let selected = opts.theme?.selected ?? "novaclaw"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "novaclaw.default",
