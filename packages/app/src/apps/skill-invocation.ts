...
export interface InvocationView {
  ...
  /** The "Show it for me to run" switch position — the user’s own answer, and what the slash menu does. */
  readonly me: boolean
  /**
   ...
   *
   * With one layer this is both the label the preset buttons compare against and what is in force.
   */
...
export function invocationOf(input: InvocationInput): InvocationView {
  ...
  const seen = SkillInvocation.visibility(input.store, input.name)
  const me = seen.instance
  // ⚠️ The PRESET names the pair of SWITCHES, which is what the two controls on this page say. It is
  // also what is in force now that nothing else can hide a skill.
  const state = { nova, me }
  return {
    nova,
    me,
