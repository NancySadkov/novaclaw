...
describe("how a colleague's model is written down", () => {
  test("round-trips provider and model", () => {
    expect(modelRef({ providerID: "spark-holo", modelID: "holo3.1" })).toBe("spark-holo/holo3.1")
    expect(parseModelRef("spark-holo/holo3.1")).toEqual({ providerID: "spark-holo", modelID: "holo3.1" })
  })
  ...
  test("a model id containing a slash still parses on the FIRST slash", () => {
    ...
    expect(parseModelRef("hf/meta-llama/Llama-3")).toEqual({ providerID: "hf", modelID: "meta-llama/Llama-3" })
  })
