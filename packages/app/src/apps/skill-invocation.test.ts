...
describe("the skills store does not touch the AGENT half - that is the permission rule's job", () => {
  test("hiding a skill from the user's menu leaves what agents may choose alone", () => {
    const view = read({ store: { pdf: { show: false } } })
    expect(view.me).toBe(false)
    expect(view.nova).toBe(true)
    expect(view.blockedElsewhere).toBe(false)
  })
})
