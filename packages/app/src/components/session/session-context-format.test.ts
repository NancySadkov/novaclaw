...
// formatter can be an ISO string, not epoch millis — older event/cache inputs and native message
// times aren't run through `normalizeSessionTimes`. `DateTime.fromMillis` threw on the string
// ("fromMillis requires a numerical input …"), faulting the whole context tab. The formatter must
// tolerate every supported representation.
describe("createSessionContextFormatter.time", () => {
  ...
  it("tolerates an ISO string without throwing", () => {
  ...
  it("keeps epoch zero as a real instant", () => {
    expect(fmt.time(0)).not.toBe("—")
  })
})
