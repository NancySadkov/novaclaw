...
describe("buildRequestParts", () => {
    ...
    expect(
      result.requestParts.some(
        (part) =>
          part.type === "text" &&
          part.synthetic &&
          part.metadata?.novaclawComment &&
          (part.metadata.novaclawComment as { comment?: string }).comment === "check this",
      ),
    ).toBe(true)
  ...
  test("preserves an external attachment source path for the model", () => {
    const result = buildRequestParts({
      ...
      images: [
        {
          type: "image",
          id: "img_external",
          filename: "novaclaw.global.dat",
          sourcePath: "C:\\Users\\Luke\\AppData\\Roaming\\ai.novaclaw.desktop.beta\\novaclaw.global.dat",
          mime: "text/plain",
      ...
      sessionDirectory: "C:\\Repos\\sst\\novaclaw",
    })

    expect(result.requestParts.find((part) => part.type === "file")?.filename).toBe(
      "C:\\Users\\Luke\\AppData\\Roaming\\ai.novaclaw.desktop.beta\\novaclaw.global.dat",
    )
  })
  ...
  test("handles macOS paths correctly", () => {
    ...
    const result = buildRequestParts({
      ...
      messageID: "msg_mac_1",
      sessionID: "ses_mac_1",
      sessionDirectory: "/Users/kelvin/Projects/novaclaw",
    })
    ...
    if (filePart?.type === "file") {
      ...
      expect(() => new URL(filePart.url)).not.toThrow()
      // Should be a normal Unix path
      expect(filePart.url).toBe("file:///Users/kelvin/Projects/novaclaw/README.md")
    }
  })
