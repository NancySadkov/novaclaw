...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("novaclaw://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("novaclaw://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("novaclaw://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("novaclaw://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("novaclaw://open-project")).toBeUndefined()
    expect(parseDeepLink("novaclaw://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "novaclaw://open-project?directory=/a",
      "novaclaw://other?directory=/b",
      "novaclaw://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("novaclaw://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("novaclaw://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("novaclaw://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("novaclaw://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "novaclaw://new-session?directory=/a",
      "novaclaw://open-project?directory=/b",
      "novaclaw://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __NOVACLAW__: {
        deepLinks: ["novaclaw://open-project?directory=/a"],
      },
    } as unknown as Window & { __NOVACLAW__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["novaclaw://open-project?directory=/a"])
