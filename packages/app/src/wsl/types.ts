...
export type WslJob =
  ...
  | { kind: "probe-novaclaw"; distro: string; startedAt: number }
  | { kind: "install-novaclaw"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeNovaclaw(name: string): Promise<void>
  installNovaclaw(name: string): Promise<void>
