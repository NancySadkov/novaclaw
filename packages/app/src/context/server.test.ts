  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://novaclaw.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://novaclaw.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://novaclaw.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://novaclaw.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://novaclaw.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://novaclaw.example.com"),
      ),
    ).toEqual({
