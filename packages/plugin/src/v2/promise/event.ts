...
export interface EventHooks {
   ...
   *
   * The HOST owns the fork, the failure isolation and the teardown — a handler that rejects
   * or throws is logged and the subscription survives, and one plugin's bad handler never
   * starves another's. Delivery is buffered per subscription and DROPS (with a log) once a
   * handler falls far enough behind, so a slow plugin cannot grow the instance's memory
   * without bound.
   *
   ...
   *
   * The handler is awaited, so it must return a promise — write `async (event) => {}` even
   * when the body is synchronous.
   */
  readonly subscribe: <Type extends EventType>(
    type: Type,
    handler: (event: EventMap[Type]) => Promise<void>,
