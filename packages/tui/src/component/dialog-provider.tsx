...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@novaclaw/sdk/v2"
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__novaclaw_custom_provider__"
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in novaclaw.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in novaclaw.json to use it.`,
          })
          dialog.clear()
          return
        }
