import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@novaclaw/plugin/tui"
...
import type { useNovaclawKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useNovaclawKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getNovaclawModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getNovaclawModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
