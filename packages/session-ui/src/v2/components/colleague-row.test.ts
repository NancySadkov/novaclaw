...
describe("the colleague row names who, and what was said", () => {
  test("a message leads with the recipient", () => {
    expect(colleagueRow({ op: "ask", colleague: "aris", message: "Where is the ledger?" })).toEqual({
      title: "Messaged aris",
  ...
  test('"Messaged", never "Asked" — the same op carries the answer', () => {
    ...
    const meta = colleagueRow({ op: "ask", colleague: "doriel", message: "Behind the clock." })
    expect(meta.title).toBe("Messaged doriel")
    expect(meta.title).not.toContain("Asked")
  })

  test("staffing reads as staffing", () => {
    expect(colleagueRow({ op: "hire", title: "Bookkeeper" }).title).toBe("Hired a colleague")
    expect(colleagueRow({ op: "retire", colleague: "aris" })).toEqual({
      title: "Retired a colleague",
      subtitle: "aris",
    })
    expect(colleagueRow({ op: "list" }).title).toBe("Looked up colleagues")
  })

  test("a nameless recipient is still legible, never blank", () => {
    expect(colleagueRow({ op: "ask" }).title).toBe("Messaged a colleague")
  })
  ...
  test("NEGATIVE CONTROL: a subtitle is omitted, never blank", () => {
    ...
    expect(colleagueRow({ op: "ask", colleague: "aris", message: "" })).toEqual({ title: "Messaged aris" })
    expect(colleagueRow({})).toEqual({ title: "Messaged a colleague" })
  })
})
