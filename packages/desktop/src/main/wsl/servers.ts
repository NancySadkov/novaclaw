...
import { expectNovaclawVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveNovaclaw?: typeof resolveWslNovaclaw
...
export function createWslServersController(
  ...
  const setNovaclawCheck = (distro: string, check: WslNovaclawCheck) => {
    setState({
      novaclawChecks: {
        ...state.novaclawChecks,
        [distro]: check,
      },
    })
  }

  const checkNovaclaw = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveNovaclaw ?? resolveWslNovaclaw)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return novaclawCheck(distro, resolved, version, appVersion)
  }

  const refreshNovaclawCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setNovaclawCheck(distro, await checkNovaclaw(distro, opts))
  }
  ...
  const refreshNovaclawCheckBackground = (id: string, distro: string) => {
    void checkNovaclaw(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setNovaclawCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl novaclaw check failed", { id, distro, message })
      })
  }

  const refreshNovaclawChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkNovaclaw(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setNovaclawCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl novaclaw check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshNovaclawCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async probeNovaclaw(name: string) {
      await runJob({ kind: "probe-novaclaw", distro: name, startedAt: Date.now() }, async (abort) => {
        await refreshNovaclawCheck(name, { signal: abort.signal })
      })
    },

    async installNovaclaw(name: string) {
      await runJob({ kind: "install-novaclaw", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslNovaclaw(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "NovaClaw installation failed")
        }
        await refreshNovaclawCheck(name, { signal: abort.signal })
        expectNovaclawVersion(state.novaclawChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.novaclawChecks, distro) : {}),
      })
    },
...
function novaclawCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "novaclaw is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "novaclaw is installed but could not run",
    }
  }
  return {
    distro,
